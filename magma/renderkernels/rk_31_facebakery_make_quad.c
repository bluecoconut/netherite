/* rk_31_facebakery_make_quad.c - compute core of render-opt kernel 31_facebakery_make_quad.
 * All helpers (rotate chain, getFacingFromVertexData, applyFacing, fillNormal, UV helpers) and
 * the makeBakedQuad body copied VERBATIM from candidate.c; only main()/stdio removed. SCALE_22_5/
 * SCALE_GEN computed lazily. Build with -ffp-contract=off. */
#include "rk.h"
#include <math.h>

static float b2f(uint32_t b) { float f; __builtin_memcpy(&f, &b, sizeof f); return f; }
...
static void applyFacing(int32_t *d, int face) {
    int32_t cp[28];
    __builtin_memcpy(cp, d, sizeof cp);
    float af[6];
...
int rk_facebakery_make_quad(float fx, float fy, float fz,
    ...
    __builtin_memcpy(out_d, d, sizeof d);
    return enumfacing;
}
