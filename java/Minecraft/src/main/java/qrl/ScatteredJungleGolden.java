...
/** Actual 1.11.2 JunglePyramid volume, metadata, and loot-seed oracle. */
public final class ScatteredJungleGolden {
    private static final class MemoryWorld extends World {
        ...
        MemoryWorld() {
            super(new SaveHandlerMP(),
                new WorldInfo(new WorldSettings(0L, GameType.SURVIVAL, true,
                    false, WorldType.DEFAULT), "jungle-oracle"),
        ...
        public boolean setBlockState(BlockPos pos, IBlockState state, int flags) {
            ...
            if (state.getBlock() == Blocks.CHEST) tile = new TileEntityChest();
            else if (state.getBlock() == Blocks.DISPENSER)
                tile = new TileEntityDispenser();
            if (tile != null) {
                tile.setWorld(this);
                tile.setPos(key);
                tiles.put(key, tile);
    ...
    private static long lootSeed(TileEntity tile) {
    ...
    private static void run(long seed) {
        ...
        ComponentScatteredFeaturePieces.JunglePyramid pyramid =
            new ComponentScatteredFeaturePieces.JunglePyramid(random, 0, 0);
        StructureBoundingBox bounds =
            new StructureBoundingBox(-64, 0, -64, 64, 255, 64);
        if (!pyramid.addComponentParts(world, random, bounds))
            throw new AssertionError("jungle pyramid placement failed");
        ...
        List<Map.Entry<BlockPos, TileEntity>> sites =
        ...
        for (Map.Entry<BlockPos, TileEntity> entry : sites) {
            BlockPos pos = entry.getKey();
            IBlockState state = world.getBlockState(pos);
            char kind = entry.getValue() instanceof TileEntityDispenser ? 'D' : 'C';
            System.out.printf("%c %d %d %d %d %d%n", kind,
                pos.getX(), pos.getY(), pos.getZ(),
                state.getBlock().getMetaFromState(state),
                lootSeed(entry.getValue()));
        }
    }
