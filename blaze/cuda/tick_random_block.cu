/* CUDA driver for tick_random_block - same core as CPU path. */
#include <cstdio>
#include <cstdlib>
...
__global__ void run_trb(Env *e, TrbAux *aux, u64 seed, TrbEmitCtx *lines, int *n_lines) {
    int t;
...
int main(int argc, char **argv) {
    ...
    Env *d_e = NULL;
    TrbAux *d_aux = NULL;
    TrbEmitCtx *d_lines = NULL;
    int *d_n = NULL;

    if (cudaMalloc(&d_e, sizeof(Env)) != cudaSuccess ||
        cudaMalloc(&d_aux, sizeof(TrbAux)) != cudaSuccess ||
        cudaMalloc(&d_lines, sizeof(TrbEmitCtx) * TWC_NTICKS * 3) != cudaSuccess ||
    ...
    for (si = 0; si < n_seeds; ++si) {
        ...
        TrbEmitCtx h_lines[TWC_NTICKS * 3];
        int n = 0;
        int i;

        run_trb<<<1, 1>>>(d_e, d_aux, seed, d_lines, d_n);
        {
