/* CUDA driver for tick_fluid_ca - same core as CPU path. */
#include <cstdio>
#include <cstdlib>
...
__global__ void run_tfc(Env *e, u64 seed, u16 *cur, u16 *tmp, TfcEmitCtx *lines, int *n_lines) {
    int t;
...
int main(int argc, char **argv) {
    ...
    if (cudaMalloc(&d_e, sizeof(Env)) != cudaSuccess ||
        ...
        cudaMalloc(&d_lines, sizeof(TfcEmitCtx) * TWC_NTICKS * 3) != cudaSuccess ||
    ...
    for (si = 0; si < n_seeds; ++si) {
        ...
        TfcEmitCtx h_lines[TWC_NTICKS * 3];
        int n = 0;
        int i;

        run_tfc<<<1, 1>>>(d_e, seed, d_cur, d_tmp, d_lines, d_n);
        {
