/* CUDA driver for populate_ice_snow - same core as CPU. */
#include <cstdio>
#include <cstdlib>
#include "../core/populate_ice_snow.h"

__global__ void run_pis(i64 seed, McSinTable *st, World *w, u16 *blocks,
    ...
    w->st = st;
    w->blocks = blocks;
    pis_run(w, sc, primer, &r, fol, seed, sky, blk, tmp_sky, tmp_blk);
}
...
int main(int argc, char **argv) {
    ...
    run_pis<<<1, 1>>>(seed, d_st, d_w, d_blocks, d_sky, d_blk, d_tmp_sky, d_tmp_blk,
                      d_sc, d_primer, d_fol);
